...
const colorVariants: Record<ColorVariant, { outer: string; inner: string; button: string; textColor: string; textShadow: string }> = {
  ...
  gold: {
    ...
    textShadow: "[text-shadow:_0_1px_0_rgb(255_240_150_/_60%)]",
  },
  bronze: {
...
const metalButtonVariants = (variant: ColorVariant = "default", isPressed: boolean, isHovered: boolean, isTouchDevice: boolean) => {
  ...
  return {
    ...
    innerStyle: { transition: t, filter: isHovered && !isPressed && !isTouchDevice ? "brightness(1.05)" : "none" },
    ...
    buttonStyle: { transform: isPressed ? "scale(0.97)" : "scale(1)", transition: t, filter: isHovered && !isPressed && !isTouchDevice ? "brightness(1.02)" : "none" },
  }
}
